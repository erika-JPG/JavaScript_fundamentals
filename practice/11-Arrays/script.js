...
btnTransfer.addEventListener('click', function (e) { 
  ...
  if (amount > 0 &&
    receiverAcc && currentAccount.balance >= amount && receiverAcc?.username !== currentAccount.username) { 
...
btnClose.addEventListener('click', function (e) { 
  ...
  if (currentAccount.username === inputCloseUsername.value && currentAccount.pin === Number(inputClosePin.value)) { 
    ...
    //The splice methods mutates the accounts array and there is no need to store this in a separate value.
    // Delete Account
...
//#3 Creating a new object wich contains the sum of the deposits and withdrawals

// in arrow functions the value is implicitly returned when we don't have a function body (curly braces), so we need to manually return the value
const {dpsts, wthwls} = accounts.flatMap(acc => acc.movements).reduce((sums, curr) => { 
  // curr > 0 ? sums.dpsts += curr : sums.withrawals += curr;
  //  fixing the duplication from above
  sums[curr > 0 ? 'dpsts' : 'wthwls'] += curr;
  return sums;
...
//   e.recommendedFood = recommendedFood;
...
// const saraDog = dogs.forEach(e => {
//   if (e.owners.includes('Sarah')) {
//     const range = e.recommendedFood - e.curFood < 0 ? `too much` : `too little`;
//     console.log(`Sarah's dog is eating ${range}`)
//   }
// });
...
// dogs.forEach(e => {
//   e.recommendedFood - e.curFood < 0 ? ownersEatTooMuch.push(e) : ownersEatTooLittle.push(e);
// })
...
//   let quantity = '';
//   array.forEach(e => {
//     quantity = e.recommendedFood - e.curFood < 0 ? `too much` : `too little`;

//     ownList.push(e.owners)
//   })
//   const allOwners = ownList.flatMap(own => own).join(' and ').trim();
//   console.log(`${allOwners}'s dogs eat ${quantity}`);
// }

// owners(ownersEatTooMuch);
// owners(ownersEatTooLittle);
...
// dogs.forEach(e => {
//   const quantity = e.recommendedFood - e.curFood == 0 ? true : false;
...
// dogs.forEach(e => {
//   const quantity = ((e.current > (e.recommendedFood * 0.90)) && (e.current < (recommendedFood *
//     1.10)))  ? true : false;
//   console.log(quantity);
// })
